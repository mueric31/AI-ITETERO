...
def ask_llm_with_context(client: OpenAI, context: str, question: str) -> str:
    ...
    CRITICAL: Must use PDF content EXACTLY as written, not paraphrase or generate own answer.
    """
    system = (
        "Uri umufasha uvuga Kinyarwanda. SUBIZA IKIBAZO UKORESHEJE **GUSA** AMAKURU ARI MU CONTEXT.\n\n"
        
        "ITEGEKO RY'INGENZI:\n"
        "- Niba CONTEXT ifite igisubizo → KORESHA icyo gisubizo NK'UKO KIRI (use exactly as written)\n"
        "- NTUGAHINDURE amagambo, NTUWONGEREMO ibyo utari kubona, NTUSOBANURE mu magambo yawe\n"
        "- Koresha amagambo, urutonde (•), n'amanomero nk'uko byanditswe mu CONTEXT\n"
        f"- GUSA niba CONTEXT idafite igisubizo CY'UKURI → subiza '{FALLBACK}'\n\n"
        
        "INGINGO Z'AKAMARO:\n"
        "- Niba hari urutonde (•) mu CONTEXT → koresha urutonde nk'uko rwanditse\n"
        "- Niba hari amanomero (1, 2, 3) → koresha amanomero nk'uko yanditse\n"
        "- Niba hari ibimenyetso byerekana ikintu → koresha ibyo bimenyetso\n"
        "- NTUGASOHORE mu CONTEXT ugahanga ibisubizo byawe\n\n"
        
        "Urugero:\n"
        "CONTEXT: '• Arasama ubundi akajya akebuka'\n"
        "Igisubizo cyawe: '• Arasama ubundi akajya akebuka' (NK'UKO KIRI)\n"
        "SIBYO: '1. Gukora ku gatuza' (NTABWO ARI NEZA - wahinduje!)\n\n"
        
        "Icyitonderwa: 'Umuhondo' bishobora kuba amashereka CYANGWA indwara. "
        "Tanga icyo CONTEXT ivuga."
    )
    messages = [
        {"role": "system", "content": system},
    ...
    resp = client.chat.completions.create(
        model=CHAT_MODEL,
        messages=messages,
        temperature=0.05,  # Very low temperature for faithful reproduction
    )
...
def get_response(question: str) -> str:
    ...
    if chunks and len(chunks) > 0:
        ...
        # Trust PDF answer as long as it's not the fallback - don't filter by length
        if pdf_answer and pdf_answer.strip() != FALLBACK.strip():
            return pdf_answer
